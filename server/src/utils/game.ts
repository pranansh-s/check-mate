import { Game, GameType } from '@xhess/shared/types';
...
export const updateTimeLeft = (game: Game, isMove: boolean) => {
  ...
  if (game.playerTurn === 'white') {
    if (game.whiteSidePlayer) {
      game.whiteSidePlayer.remainingTime = Math.max(
        game.whiteSidePlayer.remainingTime - timeElapsed + (isMove ? timeToAdd : 0),
        0
      );
    }
  } else {
    if (game.blackSidePlayer) {
      game.blackSidePlayer.remainingTime = Math.max(
        game.blackSidePlayer.remainingTime - timeElapsed + (isMove ? timeToAdd : 0),
        0
      );
    }
  }
